...
x0 = list(filter(lambda x: x[-1] == 0.0, data))   #filter 为滤波器
...
if torch.cuda.is_available():
    print('GPU计算！！！！！！！！！！！！')
    x = torch.tensor(x_data).cuda()
    y = torch.tensor(y_data).duda()
else:
    print('普通计算！！！！！！！！！！！！')
...
for epoch in range(50000):
    ...
    correct = (abs(mask-y) < 0.1).sum()
    acc = int(correct) / x.size(0)
    #print(x.size(0))
    #backward
    optimizer.zero_grad()
    loss.backward()
...
plt.plot(list(plot_x), list(plot_y))
plt.show()
